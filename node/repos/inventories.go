...
// InventoryRepo is a simple implementation of the inventory store.
type InventoryRepo struct {
	inventories map[types.Hash]*types.Inventory
}

// NewInventoryRepo creates a new store for block inventories.
...
func (ir *InventoryRepo) Add(inv *types.Inventory) error {
...
func (ir *InventoryRepo) Has(hash types.Hash) bool {
	_, ok := ir.inventories[hash]
	return ok
}

// Get retrieves the inventory with the given block hash.
func (ir *InventoryRepo) Get(hash types.Hash) (*types.Inventory, error) {
